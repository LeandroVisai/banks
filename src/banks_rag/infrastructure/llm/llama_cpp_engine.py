...
# Re-exports por compatibilidad (tests e imports existentes usan los nombres
# con guion bajo). La implementación vive en ``_common.py``, compartida con
# ``OpenAICompatEngine``.
_strip_think = strip_think
_detect_family = detect_family
_parse_args = parse_args
_format_tools_as_text = format_tools_as_text
_flatten_for_gemma = flatten_for_gemma
