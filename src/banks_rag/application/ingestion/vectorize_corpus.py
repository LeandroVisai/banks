...
Política multimodal:
  - Si el embedder es VL: usa ``encode_image`` para chunks con ``image_path``.
  - Si no, o falla la imagen: cae al embedding del texto descriptivo
...
def _embed_image_chunks(
    ...
    """Embebe chunks con ``image_path``. Cae al embedding de texto si falla."""
    out: dict[int, np.ndarray] = {}
    for i in image_idxs:
        chunk = chunks[i]
        emb: np.ndarray | None = None
        if embedder.is_multimodal:
            emb = embedder.encode_image_safe(chunk.get("image_path", ""))
        if emb is None:
            text = build_embed_text(chunk, embedder.name)
            emb = embedder.encode_text([text], batch_size=1)[0]
        out[i] = emb
    return out
...
def vectorize_corpus(
    ...
    *,
    ...
    # ── Imágenes: embed individual con fallback a texto ───────────────────────
    image_embeddings = _embed_image_chunks(embedder, chunks, image_idxs)
