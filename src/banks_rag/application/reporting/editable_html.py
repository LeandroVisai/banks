...
_EDITABLE_JS = r"""(function(){
  function dl(h,n){var a=document.createElement('a');a.href=URL.createObjectURL(new Blob([h],{type:'text/html'}));a.download=n;a.click();}
  function base(){return (document.title||'reporte').replace(/[^\w.-]+/g,'_').slice(0,80);}
  function mk(){
    ...
    var bar=document.createElement('div');bar.id='cb-save-widget';bar.setAttribute('data-cb-chrome','');
    ...
    function fin(){var c=document.documentElement.cloneNode(true);
      c.querySelectorAll('#cb-save-widget,#cb-editable-script,[data-cb-chrome]').forEach(function(el){el.remove();});
...
def make_editable_html(html: str) -> str:
    """Inyecta el panel editable (💾/📄) antes de ``</body>``. Idempotente: si el
    HTML ya lo trae, lo devuelve sin cambios. No modifica el contenido del informe;
    la edición ocurre en el navegador."""
...
_RE_EDIT_SCRIPT = re.compile(
    r'<script id="(?:cb-editable-script|cb-save-script)"[^>]*>.*?</script>', re.S
)
_RE_SAVE_WIDGET = re.compile(r'<div id="cb-save-widget"[^>]*>.*?</div>', re.S)
_RE_CHROME_NODE = re.compile(r'<[a-zA-Z]+[^>]*\sdata-cb-chrome[^>]*>.*?</[a-zA-Z]+>', re.S)
_RE_CONTENTEDITABLE = re.compile(r'\s+contenteditable="[^"]*"')
# El contorno de edición que el JS aplica en runtime (``el.style.outline=...``); el
# navegador puede serializar el color como rgb(...). Se quita de cualquier ``style``.
_RE_OUTLINE = re.compile(r'\s*outline(?:-offset)?\s*:[^;"]*;?')
# ``style`` que quedó vacío tras quitar el contorno (era el único declarante): se
# elimina para que un inliner posterior no lo confunda con "ya tiene estilo".
_RE_EMPTY_STYLE = re.compile(r'\s+style="\s*"')


def strip_editable_chrome(html: str) -> str:
    """Quita el panel flotante, el script editable y los ``contenteditable`` +
    contorno de edición. Deja el informe limpio (equivale a "Versión final")."""
    ...
    return _RE_EMPTY_STYLE.sub("", html)
