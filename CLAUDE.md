...
- **Fidelidad del correo: rasterizar, no traducir CSS** (`reports_to_eml2.py`): Outlook de escritorio renderiza con el motor de **Word**, que ignora el `<style>` del `<head>` y no entiende `grid`, `max-width`, `overflow`, `calc()/min()` ni SVG. Traducir ese CSS a mano (el mapa `clase → estilo` del `reports_to_eml.py` v1) siempre se queda corto — deja afuera `body` (todo cae a Times New Roman) y las tablas, que dependen justo de lo que Word descarta. El v2 tiene dos caminos: `--mode pixel` rasteriza el informe con el navegador del sistema (`reporting/headless.py`, Chrome/Edge por CLI, sin dependencias nuevas) y manda esas tiras como `cid:`, con lo cual la fidelidad es exacta por construcción; `--mode hibrido` vuelca el CSS con un inliner **genérico** que parsea el `<style>` real (`reporting/email_css.py`: selectores, especificidad, `var()` de `:root`) y rasteriza solo gráficos y tablas. **El cuerpo es FLUIDO**: nada lleva ancho fijo en píxeles ni `max-width` — el correo se acomoda al panel de lectura de cada destinatario. Las imágenes viajan con ancho en **porcentaje del hueco** que ocupan en el informe (Word entiende `width="63%"` en una tabla), que es lo único que conserva la proporción sin provocar scroll horizontal al angostar el panel. Un `max-width` acá sería peor que inútil: Word lo ignora igual y solo lograría que la vista previa del HTML plano mienta sobre cómo se ve el correo.

  Tres acoplamientos a cuidar: (1) `headless.py` **no** puede usar `--dump-dom` ni `--virtual-time-budget` (cuelgan en `--headless=new`), espera el chunk `IEND` del PNG en vez de esperar al proceso, y en la captura por lotes le pasa el fondo EXPLÍCITO — deducirlo de la última fila se come el separador de cierre y con él el último fragmento. (2) `reports_to_eml2.py` rasteriza el `<div class="block-table">` ENTERO, no cada `<table>`: el ancho de una tabla no se deduce de sus clases porque cada wrapper trae su propio inline (`max-width:100%` con `width:auto`, una grilla de 4 tablas, `max-width:760px`), y adivinarlo cortaba columnas. (3) `_GUTTER`/`_GAP`/`_CARD_PAD` replican el CSS de `curated_report.py` para saber el ÁREA de cada bloque; si cambia la grilla del informe hay que actualizarlos.
