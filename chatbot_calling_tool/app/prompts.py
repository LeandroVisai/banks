...
Diferencias clave vs chatbot/prompts.py:
  - NO recibe contexto pre-cargado: el agente debe BUSCAR todo lo que necesite.
  - Instruye al modelo sobre cómo planificar, llamar tools, y cuándo parar.
  - Prohibe explícitamente responder antes de haber buscado.

Las definiciones de tools NO van aquí — `apply_chat_template(tools=...)` las
inyecta automáticamente desde `tools/registry.py`.
"""
