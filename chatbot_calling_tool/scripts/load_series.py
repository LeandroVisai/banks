#!/usr/bin/env python3
"""
load_series.py — Carga datos de series históricas desde CSV.

Idéntico al equivalente en chatbot/ (las tablas `historical_series`,
`historical_data` son compartidas entre ambas infraestructuras).
...
async def run(series_id: str, csv_path: Path, replace: bool) -> None:
    ...
    try:
        if not await _series_exists(series_id):
            raise SystemExit(
                f"La serie '{series_id}' no está en `historical_series`. "
